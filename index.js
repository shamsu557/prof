      ...
      bcrypt.hash(password, saltRounds, (err, hashedPassword) => {
        ...
        db.query('INSERT INTO admins (username, password, email, fullName, phone) VALUES (?, ?, ?, ?, ?)', 
          [username, hashedPassword, email, fullName, phone], (err) => {
            ...
            res.json({ success: true, message: 'Admin created successfully! They can now access the dashboard', redirectUrl: '/admin-login.html' });
          }
        );
      });
    }
...
app.get('/files/:fileName', (req, res) => {
  const filePath = path.join(__dirname, req.params.fileName); // Ensure the file exists in the directory
